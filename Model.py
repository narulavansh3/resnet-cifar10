...
class Cifar(object):
	...
	def setup(self, training):
		...
		if training:
			...
			### YOUR CODE HERE
			# cross entropy
			cross_entropy = tf.losses.sparse_softmax_cross_entropy(logits=logits, labels=self.labels)
			# final loss function
			self.losses = cross_entropy + l2_loss
	...
	def train(self, x_train, y_train, max_epoch):
		...
		for epoch in range(1, max_epoch+1):
			...
			for i in range(num_batches):
				...
				# Run
				feed_dict = {self.inputs: x_batch_new,
							self.labels: y_batch,
	...
	def test_or_validate(self, x, y, checkpoint_num_list):
		...
		for checkpoint_num in checkpoint_num_list:
			...
			for i in tqdm(range(x.shape[0])):
				### YOUR CODE HERE
				x_preprocessed = parse_record(x[i], False)
				feed_dict = {self.inputs: x_preprocessed.reshape(1,32,32,3),
							self.labels: y[i]}
			...
			print('Test accuracy: {:.4f}'.format(np.sum(preds==y)/y.shape[0]), preds)
